...
for(semestre in semestres){
  ...
  for(coluna in 1:length(names(serie_retornos_normalizado))){
    for(incremento in incremento_fdp){
      ...
      grupo_coeficiente_B = rbind(grupo_coeficiente_B,c(coeficiente_B,sse,volatilidade,incremento,tamanho_serie))
      colnames(grupo_coeficiente_B) = c("coeficiente_B","sse","volatilidade","incremento","tamanho_serie")
...
grupo_coeficiente_B = grupo_coeficiente_B[,-1]
boxplot(grupo_coeficiente_B$coeficiente_B)
...
# write.table(grupo_coeficiente_B,"por_semestre_b_volatilidade_incremento_sse.csv",row.names=F,sep=",")
