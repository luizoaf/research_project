...
# filtra a serie de retornos pelo ano e depois faz a media por setor
...
# faz media das acoes por setor e depois cria a serie de retornos
media_das_series_de_retornos_das_acoes_por_setor = function(ano){
  #   ano = 2008
  ano_acoes = subset(dados,dados$datas==ano)
  #   head(ano_acoes)
  #   serie_retornos_por_ano = cria_tabela_serie_retornos_de_todas_as_acoes(ano_acoes)
  #   head(serie_retornos_por_ano)
  setores_100_porcento = setores_100_porcento_por_periodo(ano)
  setores_media_acoes = data.frame(1)
  relacao_setores_acoes_menos_acoes = relacao_setores_acoes(dados,ano)
  ...
  for( setor in setores_100_porcento){
    acoes_do_setor = paste(relacao_setores_acoes_menos_acoes$codigo[relacao_setores_acoes_menos_acoes$setores==setor],".SA",sep="")
    if(length(acoes_do_setor)!=1){ # nao ira calcular a media quando tiver apenas 1 acao
      medias_por_setor = apply(ano_acoes[,acoes_do_setor],MARGIN=1,FUN=mean)
    }else{
      medias_por_setor = ano_acoes[,acoes_do_setor]
    }
    setores_media_acoes = cbind(setores_media_acoes,medias_por_setor)
  }
  #   setores_media_acoes = setores_media_acoes[,-1]
  # a coluna que seria so de um para criar o data frame foi eliminada da mesma forma que e eliminada a data, cortando a primeira coluna pela funcao cria_tabela_serie_retornos_de_todas_as_acoes
  serie_retornos_por_ano = cria_tabela_serie_retornos_de_todas_as_acoes(setores_media_acoes)
  colnames(serie_retornos_por_ano) = setores_100_porcento_por_periodo(ano)
  return(serie_retornos_por_ano)
}
...
setores_das_series_de_retorno=c()
# indice_setores_das_series_de_retorno =1
# 
...
for(ano in faixa_temporal){
  ...
  serie_retornos_normalizado = janelamento(serie_retornos_normalizado,tamanho_janelamento) # cria coluna grupo, que define o janelamento
  for(coluna_setor in 1:(length(names(serie_retornos_normalizado))-1)){
    for(grupo in unique(serie_retornos_normalizado$grupo)){
      #       coluna_setor = 1
      #       grupo =1
      coluna_setores[i] = names(serie_retornos_normalizado)[coluna_setor]
      faixa_temporal_por_setores[i] = ano
      serie = serie_retornos_normalizado[serie_retornos_normalizado$grupo==grupo,coluna_setor]
      ...
      setores_das_series_de_retorno[i] = names(serie_retornos_normalizado)[coluna_setor]
...
# adicionar coluna_setores(setores) e faixa_temporal_por_setores(anos) no resultado final com o nome dos setores
grupo_coeficiente_B = cbind(faixa_temporal_por_setores,grupo_coeficiente_B)
grupo_coeficiente_B = cbind(data.frame(setores=coluna_setores),grupo_coeficiente_B)
grupo_coeficiente_B$b_volatilidade = grupo_coeficiente_B$coeficiente_B*grupo_coeficiente_B$volatilidade
write.table(grupo_coeficiente_B,paste("b_volatilidade_janelemanto_",tamanho_janelamento,"_dias_por_setor_com_ano.csv",sep=""),dec=".",row.names=F)

# grupo_coeficiente_B = read.table("b_volatilidade_janelemanto_30_dias_por_setor_com_ano.csv",head=T)
# plot(grupo_coeficiente_B$b_volatilidade,ylim=c(0,max(grupo_coeficiente_B$b_volatilidade)),ylab="Volatility x Coefficient B",xlab="Indices")
...
# 
# 
# plot((grupo_coeficiente_B$coeficiente_B[order(grupo_coeficiente_B$coeficiente_B,decreasing=T)]),xlab="Volatility",ylab="Coefficient B",ylim=c(0, max(grupo_coeficiente_B$coeficiente_B)),col="red")
...
# 
# 
# treino = grupo_coeficiente_B[grupo_coeficiente_B$faixa_temporal_por_setores %in% 2008:2010,]
# # plot_comportamento_volatilidade_B(treino)
# 
# 
# plot((treino$coeficiente_B[order(treino$coeficiente_B,decreasing=T)]),xlab="Volatility",ylab="Coefficient B",ylim=c(0, max(treino$coeficiente_B)),col="red")
# points((treino$volatilidade[order(treino$volatilidade,decreasing=F)]),xlab="Indice",ylab="Volatility",col="blue")
# 
# 
# # for(ano in unique(grupo_coeficiente_B$faixa_temporal_por_setores)){
# #   print(length(grupo_coeficiente_B$setores[grupo_coeficiente_B$faixa_temporal_por_setores == ano]))
# # 
# # }
# 
# 
# # 
# # 
# # anos = as.numeric(as.character(unique(grupo_coeficiente_B$faixa_temporal_por_setores)))
# # setores_100_porcento = setores_100_porcento_por_periodo(anos[1])
# # for(ano in anos){
# #   setores_100_porcento = intersect(setores_100_porcento,as.character(unique(grupo_coeficiente_B$setores[grupo_coeficiente_B$faixa_temporal_por_setores == ano])))
# # }
# 
# plot(main= paste(length(unique(treino$setores))," setores entre 2008 e 2010"),treino$b_volatilidade,xlab="Índices",ylab="Volatilidade * Coeficiente B",col="black",ylim=c(0,2.5))
# intercept = regressao.simples(1:(length(treino$b_volatilidade)),(treino$b_volatilidade))[1]
# slope = regressao.simples(1:(length(treino$b_volatilidade)),(treino$b_volatilidade))[2]
# abline(a=intercept,b =slope,col=2)
# 
# dados = treino
# k=3
...
# dados$cluster = km$cluster
# 
# plot(main= paste(length(unique(treino$setores))," setores entre 2008 e 2010\nPara K = ",k,sep =""),dados$coeficiente_B~dados$volatilidade,xlab="Volatility",ylab="Coefficient B", col = km$cluster,pch = 20, cex = 0.9)
# points(km$centers[,2]~km$centers[,1],col=1:k, pch = 3,lwd=12)
# dados$ano = treino$faixa_temporal_por_setores
# dados$setor = treino$setores
# 
# # write.table(dados,"b_volatilidade_janelemanto_30_dias_por_setor_com_ano_treinamento_clusterizado.csv",dec=".",row.names=F)
# 
# require(rpart)
# 
# arvore = rpart(cluster~  coeficiente_B+volatilidade  ,data = dados,method="class",control=rpart.control(minsplit=5, cp=0.001))
# plot(arvore,uniform=T,branch=0)
# text(arvore,digits=3,cex=0.95,font=10,pretty=0,fancy=T,fwidth=0,fheight=0)
# 
# 
# previsao.treinamento = predict(arvore,dados,type='class')
# matriz.confusao = table(dados$cluster, previsao.treinamento)
