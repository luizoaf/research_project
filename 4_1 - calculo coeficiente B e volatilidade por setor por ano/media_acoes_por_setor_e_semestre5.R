...
retorna_cluster = function(faixa_temporal){
  dados = subset(com_b_menor_que_2,com_b_menor_que_2$faixa_temporal_por_setores==faixa_temporal)
  k=2
  iter = 45
  dados = dados[,c(3,5)]
...
#   periodo = faixa_temporal[2]
  setor_faixa_temporal = cbind(data.frame(cluster = retorna_cluster(periodo)),com_b_menor_que_2[com_b_menor_que_2$faixa_temporal_por_setores==periodo,c("setores","coeficiente_B","volatilidade","faixa_temporal_por_setores" )])
  setores_faixa_temporal = rbind(setores_faixa_temporal,setor_faixa_temporal)
}
# setores_faixa_temporal

write.table(setores_faixa_temporal,"setores_faixa_temporal.csv",row.names=F,sep=",")