...
i_erros = 1
incremento = c(2,1,0.5,0.1,0.05,0.01,0.005)
...
while(T){
  ...
  #virou o lado
  if(i_erros>2 && coeficiente_B_e_erros$sse[i_erros-2] > coeficiente_B_e_erros$sse[i_erros-1] &&  coeficiente_B_e_erros$sse[i_erros-1] < coeficiente_B_e_erros$sse[i_erros]){
    i_incremento = i_incremento + 1
    incremento_atual = incremento[i_incremento]
    coeficiente_B =ifelse(is.na(incremento_atual),0,incremento_atual)
    i_incremento = i_incremento + 1
#     incremento_atual = incremento[i_incremento]
  }
  #   else if(i_erros>2 && coeficiente_B_e_erros$sse[i_erros-2]< coeficiente_B_e_erros$sse[i_erros-1] &&  coeficiente_B_e_erros$sse[i_erros-1] > coeficiente_B_e_erros$sse[i_erros]){
  #     i_incremento = i_incremento + 1
  #     incremento_atual = incremento[i_incremento]
  #     coeficiente_B =ifelse(is.na(incremento_atual),0,incremento_atual)
  #   }
  else if(i_erros>3 &&  coeficiente_B_e_erros$sse[i_erros-2]> coeficiente_B_e_erros$sse[i_erros-1]  &&  coeficiente_B_e_erros$sse[i_erros-1] < coeficiente_B_e_erros$sse[i_erros]){
    break # SSE 3 > SSE2 < SSE1
  }else{
